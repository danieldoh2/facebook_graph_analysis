...
fn djikstra_graph_transfer(filename: &str) -> Vec<Vec<usize>> {
...
fn bfs_graph_transfer(filename: &str) -> Vec<Vec<(usize, i32)>> {
    let file = File::open(filename).unwrap();
    let reader = BufReader::new(file);
    let mut graph = vec![vec![]; 4039];
...
fn main() {
    let graph = djikstra_graph_transfer("/Users/danieldoh/Desktop/facebook_combined.txt");
